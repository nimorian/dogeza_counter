class DogesController < ApplicationController
  before_action :set_doge, only: [:show, :edit, :update, :destroy, :play]
  ...
  def create
    ...
    respond_to do |format|
      if @doge.save
        ...
        format.json { render :show, status: :created, location: @doge }
      else
  ...
  def update
    respond_to do |format|
      if @doge.update(doge_params)
        ...
        format.json { render :show, status: :ok, location: @doge }
      else
  ...
  def destroy
    @doge.destroy
    respond_to do |format|
      format.html { redirect_to doges_url, notice: 'Doge was successfully destroyed.' }
  ...
  private
    ...
    def doge_params
      params.require(:doge).permit(:title, :description, :count)
    end
end
